...
dt = datetime.now().strftime("%b-%d-%Y") # date time to save the results in Excel

path_cwd = 'C:/Users/Arun/Nanogels/newimages3'

fullpath = path_cwd+'\\'+'T6_70h_08_11_23_Sample27_well_27A_2.tif'
...
def blobs(filename):
    ...
    '''
    # filename = 'T8_23_11_23_HEK293cells_Sample4_9.tif'
    fullpath = path_cwd+'\\'+filename
...
def loctif(path_cwd): # function to locate all .tif files and list them
    list_tif = os.listdir(path_cwd)
    list_tif = [x for x in list_tif if '.tif' in x]
    return list_tif

imagelist = loctif(path_cwd)
result = blobs(imagelist[3])